...
The current checkpoint is **`phase-14-cloud-run`**, merged into `main` in PR #13 (`19d4953`). **Phase 12 — cross-platform E2E — is complete. Phase 13 — Google Cloud foundations and cost safety — was completed manually by the learner. Phase 14 — Containers, Artifact Registry, and Cloud Run — is complete with learner-reported manual acceptance.** **Next curriculum phase: Phase 15 — Service identities and Secret Manager.** Phases 7–9 add guided todo creation, server-directed screens, and owner-scoped discovery/resume with revision checks and idempotent request recovery. Earlier numbered guides and checkpoints remain available.
