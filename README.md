...
The current implementation on `main` includes **Phase 10 — LLM-assisted planning**, merged in PR #7 (`e1e2cac`). Phases 7–9 add guided todo creation, server-directed screens, and owner-scoped discovery/resume with revision checks and idempotent request recovery. Earlier numbered guides and checkpoints remain available.
...
**Next: Phase 11 — interactive AI workflows with assistant-ui and AG-UI.** Its [draft spec](docs/superpowers/specs/2026-09-10-agentic-ui-design.md) and [implementation plan](docs/superpowers/plans/2026-09-10-agentic-ui.md) are available for review on `codex/phase-11-agentic-ui`. The design uses assistant-ui with a direct AG-UI connection to the existing FastAPI server, with no additional backend service. An isolated [compatibility spike](spikes/assistant-ui-ag-ui/README.md) passed fixture interactions on Expo web and iOS Simulator; full application integration is now implemented on `codex/phase-11-agentic-ui` with automated verification recorded in [Guide 11](docs/guides/11-agentic-ui.md) (497-test PostgreSQL-backed API suite, 506-test mobile suite, clean typecheck/exports; the full `pnpm quality` gate currently stops at 15 new mobile-lint errors in Phase 11 agent/auth files, recorded in Guide 11). Live-model decisions and interactive web/iOS acceptance remain pending, and Phase 11 is not merged. The [Phase 10 spec](docs/superpowers/specs/2026-09-10-llm-assisted-planning-design.md) and [implementation plan](docs/superpowers/plans/2026-09-10-llm-assisted-planning.md) remain available as the preceding design record.
