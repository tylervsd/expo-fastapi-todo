...
Enabling observability in Terraform sets `TRACE_EXPORT_ENABLED=true` on both API and worker runtimes in the same apply as the Cloud Trace API enablement and runtime writer grants, so export starts only with writer rights in place — no manual env step, and no separate export-enablement ordering to get wrong. When observability is null, neither runtime carries the variable and local runs export nothing unless explicitly enabled. Use `TRACE_SAMPLE_RATE=1.0` for the bounded acceptance window, then restore `0.1`. Verify spans arrive after a single request followed by idle time; do not rely on another request waking an exporter thread. Record export overhead and flush behavior.
