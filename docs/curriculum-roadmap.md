...
Phase numbers start at 0 to match the README, existing guides, and checkpoint convention. This revision expands the curriculum to 30 phases (0–29); it does not renumber existing guides or tags. The implementation on `main` includes Phase 14 containerization and its manual acceptance guide. Merge status does not complete outstanding manual acceptance: see the numbered guides, especially Guide 09's partial iOS record. Phase 12 is implemented with local-runs + web-CI acceptance ([guide](guides/12-cross-platform-e2e.md)); native CI is manual-only after environment-attributed failures on free-tier runners. Phase 13 cloud foundations were completed manually, and [Phase 14](guides/14-cloud-run.md) records learner-reported Cloud Run acceptance. Phases 15 and 16 were completed manually, with all acceptance checks reported passed by the learner. Phases 17 ([acceptance](guides/17-cloudflare-pages.md#acceptance-record)), 18 ([status](guides/18-terraform.md#learner-progress-and-agreed-deferrals)), and 19 ([acceptance](guides/19-continuous-delivery.md#acceptance-record)) are learner-confirmed; Phase 18 keeps agreed deferrals (recovery/destruction drills). Phase 20 is complete with learner sign-off on 2026-09-16; unobserved acceptance drills remain recorded ([guide](guides/20-cloud-tasks-scheduler.md#acceptance-record)). Phase 23 is signed off by the learner on 2026-09-23 with a narrowed scope and agreed deferrals, including C1/C2 contract releases and tabletop injects 3–4 ([acceptance](guides/23-resilience.md#acceptance-record)). Phase 26 BigQuery product analytics is signed off by the learner on 2026-09-24 with agreed deferrals ([acceptance](guides/26-bigquery-analytics.md#acceptance-record)). Phase 28 is split: [28a](guides/28a-bigquery-analysis-tools.md) (BigQuery analysis tools) is signed off by the learner on 2026-09-24 with agreed deferrals; [28b](guides/28b-hex-analytics.md) (Hex) is completed and signed off by the learner on 2026-09-25, with existing deferrals; Phases 24, 25, 27 and 29 are provisional curriculum. Phase 28a practises BigQuery's own analysis tools on Phase 26 data. Phase 28b builds on both with Hex. Phase 29 adds Datadog RUM after the Phase 21 observability foundations.
...
- **Status:** Spec, plan, and local lab implementation authorized on 2026-09-18. The full-stack encrypted-name feature, separate application key, isolated Terraform lab and synthetic-fixture runner are implemented; the learner reported completing the guide and successful application deployment, and signed off on 2026-09-20. Individual live artifacts were not independently captured See the [design](superpowers/specs/2026-09-18-cloud-kms-design.md), [plan](superpowers/plans/2026-09-18-cloud-kms.md), and [walkthrough](guides/22-cloud-kms.md).
...
- **Approved scope (2026-09-23):** In-place timed restore (RTO 30 min, RPO 24 h), `completed` → `completed_at` expand/contract with a deliberately failed read-switch release and mid-migration rollback, a PII-in-logs tabletop, and a [readiness questionnaire](guides/23-readiness-questionnaire.md). Deferred, not passed: regional HA, Security Command Center/Artifact Analysis triage, secret rotation, dependency cadence, and Phase 18 state-recovery drills. See the [spec](superpowers/specs/2026-09-23-resilience-design.md).
...
## 28a. BigQuery analysis tools

- **Learning goal:** Use BigQuery's own tools to answer and verify product questions, and judge when SQL, a notebook, or AI-assisted analysis fits.
- **Visible outcome:** The learner answers six SQL exercises on deterministic synthetic practice data with known answers, charts results in a BigQuery Studio notebook, checks data canvas (Gemini) SQL against written metric definitions, and runs their own metric on the real Phase 26 views.
- **New technology/pattern:** BigQuery Studio, `QUALIFY` and window functions, `APPROX_QUANTILES`, as-of dates instead of `CURRENT_DATE()`, Colab Enterprise notebooks with the `%%bigquery` magic, data canvas and Gemini in BigQuery, saved queries vs views.
- **Verification emphasis:** A deterministic generator (identical checksum on rerun), property checks on the synthetic data, reference answers recorded from a live run, and a written comparison of AI-generated SQL with the definitions.
- **Non-goals:** A separate Gemini project (learner choice for the sandbox; recommended at Accountable), scheduled queries or notebooks, BigQuery ML, and Hex (28b).
- **Spec:** [Phase 28a design](superpowers/specs/2026-09-24-bigquery-analysis-tools-design.md); [walkthrough](guides/28a-bigquery-analysis-tools.md).

## 28b. Hex analytics with BigQuery

- **Status:** Completed and signed off by the learner on 2026-09-25; existing deferrals remain ([acceptance record](guides/28b-hex-analytics.md#acceptance-record)).
...
- **Non-goals:** A second event-ingestion pipeline, direct analytical queries against production Cloud SQL, a customer-data platform, real-time BI guarantees, writeback to application data, and copying todo titles or prompts into dashboards.
- **Spec gate:** Approve Hex account/plan prerequisites, supported authentication and credential lifecycle, connection identity and query-billing project, dataset location, curated view contracts, metric definitions, freshness expectations, sharing scope, and query/refresh cost limits. Prefer short-lived authentication where supported; never place credentials in notebook cells or Git.
- **Spec:** [Phase 28b design](superpowers/specs/2026-09-24-hex-analytics-design.md); [walkthrough](guides/28b-hex-analytics.md).

## 29. Datadog RUM for web and iOS

- **Learning goal:** Diagnose the performance and reliability users experience in the frontend, building on Phase 21's backend observability while distinguishing diagnostic telemetry from authoritative product analytics.
- **Visible outcome:** A learner locates a deliberately slow or failing workflow in Datadog on Expo web and iOS, identifies the affected release and screen, and inspects error and request timing without exposing user content.
- **New technology/pattern:** Datadog browser and React Native RUM SDKs, explicit view/action tracking, environment and release identifiers, source maps and native symbolication where applicable, session sampling, consent state, telemetry filtering, and session identity lifecycle. Verify compatibility with the exact Expo/React Native versions before selecting SDK pins and native build configuration.
- **Verification emphasis:** Controlled error/latency fixtures on both platforms, correct view transitions, release attribution, symbolicated stack traces, sign-out identity reset, consent behavior, filtering of URLs/errors/action labels, sampling configuration, and telemetry failure that never blocks application behavior. Use synthetic accounts and inspect outgoing payloads for passwords, tokens, todo titles, prompts, and other sensitive content.
