...
Phase numbers start at 0 to match the README, existing guides, and checkpoint convention. This revision expands the curriculum to fourteen phases (0-13); it does not renumber existing guides or tags. The implementation on `main` includes Phases 7–10, with Phase 10 merged in PR #7 (`e1e2cac`). Merge status does not complete outstanding manual acceptance: see the numbered guides, especially Guide 09's partial iOS record. Phase 10 is merged; Phases 11–13 remain planned.
