...
Phase 21 owns end-to-end backend tracing and operational diagnostics. Use OpenTelemetry and durable trace context to connect the API, database operations, Cloud Tasks enqueue, worker processing, provider call, and saved result in one Cloud Trace waterfall. Preserve safe structured logs, privacy rules, native alerts, and existing provider/queue/budget controls. Verify sampling, retries, queue delay, and export under Cloud Run's CPU lifecycle. AI usage/token/cost analytics moves to Phase 26 and its dashboards to Phase 28; per-user quota enforcement is separately deferred, not part of this observability phase. Status 2026-09-17: implementation complete locally with local verification observed; live deployment and learner acceptance pending — see [Guide 21](guides/21-observability.md).
